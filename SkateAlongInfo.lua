----------------------------------------------------------------------------
--  SkateAlongInfo - display player position as color
----------------------------------------------------------------------------

SkateAlongInfo = {}
SkateAlongInfo = LibStub("AceAddon-3.0"):NewAddon("AceConsole-3.0", "AceEvent-3.0", "AceTimer-3.0", "AceComm-3.0", "AceSerializer-3.0")
...
function SkateAlongInfo:slashCommands()
    SLASH_DC1 = "/dc";
    SLASH_DC2 = "/SkateAlongInfo";
    SlashCmdList["DC"] = StartSetup;
end

-- Function to quickly log info to wow console
function SkateAlongInfo:log(msg)
...
function SkateAlongInfo:error(msg)
...
function SkateAlongInfo:GetPlayerFacing()
    local p = Minimap
...
function SkateAlongInfo:OnInitialize()
...
function SkateAlongInfo:StringToASCIIHex(str)
...
function SkateAlongInfo:CreateFrames(n)
    ...
    local function UpdateFrameColor(f)
        ...
        if not SETUP_SEQUENCE then
            ...
            MakePixelSquareArr(fixedDecimalToColor(SkateAlongInfo:GetPlayerFacing()), 3) --3 The direction the player is facing in radians
    ...
    local function setFramePixelBackdrop(f)
        f:SetBackdrop({
            bgFile = "Interface\\AddOns\\SkateAlongInfo\\white.tga",
...
function SkateAlongInfo:GetCurrentPlayerPosition()
...
function SkateAlongInfo:Base2Converter()
...
function SkateAlongInfo:MakeIndexBase2(bool, power)
...
function SkateAlongInfo:GetTargetName(partition)
    -- Uses wow function to get target string
    local target = GetUnitName("target")
    if target ~= nil then
        target = SkateAlongInfo:StringToASCIIHex(target)
        if partition < 3 then
...
function SkateAlongInfo:isInRange()
...
function SkateAlongInfo:itemName(bag, slot)
    local item
    local itemCount
...
function SkateAlongInfo:returnItemFromIndex(index)
    return items[index]
end

function SkateAlongInfo:enchantedItems()
    if ValuesAreEqual(items, itemsPlaceholderComparison) then
    end
end

function SkateAlongInfo:equipName(slot)
    local equip
...
function SkateAlongInfo:spellStatus()
    local statusCount = 0
...
function SkateAlongInfo:spellAvailable()
...
function SkateAlongInfo:notEnoughMana()
...
function SkateAlongInfo:bagSlots(bag)
    bagSlots = GetContainerNumSlots(bag)
    return bagSlots
end

-- Finds passed in string to return profession level
function SkateAlongInfo:GetProfessionLevel(skill)
...
function SkateAlongInfo:GetDebuffs(debuff)
...
-- Returns zone name
function SkateAlongInfo:GetZoneName(partition)
    local zone = SkateAlongInfo:StringToASCIIHex(GetZoneText())
...
function SkateAlongInfo:GameTime()
    local hours, minutes = GetGameTime()
    hours = (hours * 100) + minutes
    return hours
end

function SkateAlongInfo:GetGossipIcons()
...
function SkateAlongInfo:CorpsePosition(coord)
...
function SkateAlongInfo:PlayerClass()
...
function SkateAlongInfo:targetCombatStatus()
...
function SkateAlongInfo:GetEnemyStatus()
...
function SkateAlongInfo:deadOrAlive()
...
function SkateAlongInfo:checkTalentPoints()
...
function SkateAlongInfo:playerCombatStatus()
...
function SkateAlongInfo:GetBuffs(buff)
    for i = 1, 10 do
...
function SkateAlongInfo:GetInventoryBroken()
    for i = 1, 16 do
...
function SkateAlongInfo:IsPlayerFlying()
...
function SkateAlongInfo:needFood()
...
function SkateAlongInfo:needWater()
...
function SkateAlongInfo:needManaGem()
...
function SkateAlongInfo:IsTargetOfTargetPlayer()
...
function SkateAlongInfo:isUnskinnable()
...
function SkateAlongInfo:ProcessExitStatus()
...
function SkateAlongInfo:hearthZoneID()
    local index = {}
...
-- Begin Event Section -- -- Begin Event Section -- -- Begin Event Section --
-----------------------------------------------------------------------------
function SkateAlongInfo:HandleEvents()
...
function SkateAlongInfo:HandlePartyInvite()
...
function SkateAlongInfo:RepairItems()
...
function SkateAlongInfo:LearnTalents()
    if UnitCharacterPoints("player") > 0 then
...
function SkateAlongInfo:CheckTrainer()
    iterator = iterator + 1
    if Modulo(iterator, 30) == 1 then
        ...
        if GetTrainerServiceInfo(1) ~= nil and DATA_CONFIG .AUTO_TRAIN_SPELLS then
            ...
            -- 2) Have the money to train that spell
...
--the x and y is 0 if not dead
--runs the RetrieveCorpse() function to ressurrect
function SkateAlongInfo:ResurrectPlayer()
